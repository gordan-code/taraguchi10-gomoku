...
**合并 200 局 100:100，约 0 Elo。** 节点吞吐 ~2.2 倍（4 worker）但深度不变（5.4 vs 5.5）——这不是实现问题而是 root-splitting 的固有性质：切树并行降低的是**到达深度 D 的时延**，不提升**固定时控下能到达的深度**（各 worker 的子树只有单线程 1/N 大小，同墙钟时间搜到同深度）。附带发现：各子集的 aspiration 分数跨子集不可比，直接取 max 有 ~25% 局面选错着法，两阶段重验证可修复但预算翻倍不划算。**结论**：时控对弈场景 root-splitting 无收益；深度限制场景（开局库生成）可用；真正的时控增益需要 Lazy SMP（全树共享 TT 并行）——见下节。

### Lazy SMP 实验（2026-09-11，中性结果：正确性建立、增益≈0）

按上节结论实现了真正的 Lazy SMP：WASM 共享内存 + 原子置换表。**这条路在 wasm32 上可走通**（可行性实验证明：共享 `WebAssembly.Memory` 多实例、每实例 `__stack_pointer` 移栈、跨实例原子可见），但历经三轮 bug 才达到正确，最终棋力增益仍 ≈ 0：

- **架构**：Rust 内核 23 个 static 重构为 `ThreadState`（ctx 参数穿透 28 个函数）——每线程棋盘/增量表/杀手着，共享 TT/协调标志。JS 侧 `--engine wasm-smp`：共享 Memory + 主实例(tid0) + N-1 个 helper worker。
- **踩坑 1（致命）**：Rust 的 static **全部落在共享线性内存**（包括 `THREAD_BASE` 指针）——helper 的初始化会覆盖所有实例的指针，全体线程共用一个状态区互踩、幻影五连秒判 MATE。修复：ctx 参数传递 + helper 显式 tid 寻址（`extern static` 导入 wasm global 的方案实验证伪）。
- **踩坑 2（三轮 TT 设计迭代）**：共享 TT 的并发安全——v1（meta 先写）并发写同槽产生"线程1的 key 配线程2的 score"→异局面 MATE 分值秒杀主搜索（-191/-147）；v2（key+score 同一原子字）挡掉异局面混配，同局面 score/flag 错配仍在（-35）；v3（**每槽 try-lock**，CAS 失败即 miss，无自旋无死锁）彻底消除（≈0）。
- **最终结果**：20 局三轮 + 40 局确认（21:19，+17，不显著）。机制生效（4 线程总吞吐 2.5~3 倍、TT 锁定读写一致、单线程语义与 legacy 逐局面等价），但 **per-thread 吞吐因原子+锁开销塌了 ~2.5 倍**，吞掉了并行收益；主线程深度 5.6~6.2 vs 单线程 5.6~5.8。
- **结论与后续**：wasm32 共享内存 Lazy SMP 的正确性路径已铺平（ctx 穿孔架构、try-lock TT、GO/STOP 协调都保留入库）；要兑现增益需消除锁开销——分段锁/无锁 packed entry（如 128-bit 原子不可用时的 word-tearing 校验）+ helpers 更强去同步（当前仅根候选轮转）+ 跨手 TT 世代标记。当前对局部署仍是单线程内核。
